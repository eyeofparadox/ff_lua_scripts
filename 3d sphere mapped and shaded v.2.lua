-- 3d sphere mapped and shaded v.2
...
function prepare()
   ...
   -- ox,oy = 0.5, 0.5
	...
	-- sina0,cosa0 = math.sin(roll), math.cos(roll)

	-- add lighting
	mapped = get_checkbox_input(MAPPED)
...
function get_sample(x, y)
   ...
   -- local ox,oy = px,py
	...
	-- intended for screen relative rotation; produced tilt instead
	-- local nx = cosa0 * (x-ox) - sina0 * (y-oy) + ox
	-- local ny = sina0 * (x-ox) + cosa0 * (y-oy) + oy
	...
	if mapped then
		if shaded then 
			return (r*px_p)+px_p, (g*px_p)+px_p, (b*px_p)+px_p, a end
		return r, g, b, a
	else
		return h,py/2+.5,px_p,1
	end
