-- 3d perlin spherical map v.0.0.2 - improved perlin noise
function prepare()
 -- constants
...
function get_sample(x, y)
	...
	for octave_index = 1, OCTAVES_COUNT do
		...
		if mode== 2 then
			...
			ng = ng + (opacity * perlin:noise(nx_g * size + dg,ny_g * size, nz_g * size))
			nb = nb + (opacity * perlin:noise(nx_b * size, ny_b * size + db, nz_b * size))
		else	
			nr = nr + (opacity * perlin:noise(nx_r * size,ny_r * size, nz_r * size )+ dr)
	...
	elseif mode == 2 then
		...
		r, g, b, a = blend_linear_dodge(r, g, b, a, r1, g1, b1, a1, 1, hdr)
		return r, g, b, a 
	else
		-- map = true
		-- rgban = true
