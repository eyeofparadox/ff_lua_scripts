script = "_ff_cgpt_normal_and_flow_mapping.lua"
--[[
	a repository for normal and flow mapping scripts created for lua map scripts in filter forge: 
]]--
...
function get_sample(x, y)
	-- get height and surface gradient	( * height_scale)
	-- local height = get_sample_grayscale(x, y, HEIGHT) * height_scale
	local gradient_x = ((get_sample_grayscale(x + offset, y, HEIGHT) * height_scale) - 
		(get_sample_grayscale(x - offset, y, HEIGHT) * height_scale)) / (2 * offset)
	local gradient_y = ((get_sample_grayscale(x, y + offset, HEIGHT) * height_scale) - 
		(get_sample_grayscale(x, y - offset, HEIGHT) * height_scale)) / (2 * offset)
