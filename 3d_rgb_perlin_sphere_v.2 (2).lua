...
function get_sample(x, y)
	...
	distortion = get_sample_grayscale(h, py/2+0.5, DISTORTION) * 4
	...
	for octave_index = 1, OCTAVES_COUNT do
		...
		dr = dr + (opacity * noise(px_r*size,py*size,((z_r*size)+noise_z)+OUTPUT_WIDTH * -1)) * distortion
		dg = dg + (opacity * noise(px_g*size,py*size,((z_g*size)+noise_z)+OUTPUT_WIDTH * -2)) * distortion
		db = db + (opacity * noise(px_b*size,py*size,((z_b*size)+noise_z)+OUTPUT_WIDTH * -3)) * distortion
